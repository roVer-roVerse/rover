...
from rover.models.te_llama.te_llama import TELlamaForCausalLM, replace_params


def load_te_from_hf(model_name):
    ...
    te_model = TELlamaForCausalLM(config)
...
llama1b_inst = "meta-llama/Llama-3.2-1B-Instruct"


class HyperParameters:
    def __init__(self):
        self.mixed_precision = "bf16"
        self.model_name = llama1b_inst
...
def init_baseline_model(name, device="cuda"):
...
def init_te_llama_model(name, device="cuda"):
    # Init the model
...
def wrap_with_accelerator(model, hyperparams):
    ...
    optimizer = AdamW(
        params=model.parameters(), lr=hyperparams.learning_rate, fused=True
    )
...
def finetune_model(
    model, hyperparams, accelerator, train_dataloader, optimizer, lr_scheduler
):
    model.train()
    total_loss = 0
    optimizer.zero_grad()
...
def restart_jupyter_notebook():
    ...
    if torch.cuda.memory_allocated() != 0:
        ...
        warnings.warn(
            "The device memory hasn't been flushed, trying with a second method!"
        )
